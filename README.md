...
> **저장소에 `wrangler.toml` 을 두지 마세요.**
> Cloudflare 는 그 파일이 있으면 빌드 설정을 파일에서 읽고, **대시보드에 넣은
> 환경변수를 무시합니다.** 빌드 로그에 `Build environment variables: (none found)`
> 이 찍히면 이 경우입니다. 실제로 이 파일 때문에 AI 채점 키를 아무리 넣어도
> 빌드에 들어가지 않았습니다. 파일에 키를 적어 푸는 방법은 쓰면 안 됩니다 —
> 저장소에 키가 그대로 올라갑니다.
